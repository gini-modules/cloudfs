...
class Qiniu extends \Gini\Controller\API
{
    ...
    public function actionInit($serverKey)
    {
        ...
        $serverKey = $serverKey ?: 'default';
        $config = (array) $config[$serverKey];
    ...
    // 暂时不允许客户端自定义filename
    public function actionGetURI($host, $file = null)
    {
        $config = $this->_getOptions();
        $filename = \Gini\Util::randPassword().microtime();

        $result = ($config['prefix'] ?: '').sha1($host.$filename).'.'.pathinfo($file, PATHINFO_EXTENSION);
